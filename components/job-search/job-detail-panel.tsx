...
export function JobDetailPanel(props: JobDetailPanelProps) {
  if (!props.job) {
    return (
      <section className="rounded-[28px] border border-ink/8 bg-white p-6 shadow-[0_22px_60px_rgba(15,23,42,0.06)]">
        <div className="text-sm font-medium uppercase tracking-[0.18em] text-slate/65">
          Job details
        </div>
        <h3 className="mt-4 text-2xl font-semibold text-ink">Select a job to inspect it.</h3>
        <p className="mt-3 text-sm leading-6 text-slate">
          The right panel keeps the selected posting in view so you can browse the list and inspect
          details without leaving the search page.
        </p>
      </section>
    );
  }
    ...
    <section className="rounded-[28px] border border-ink/8 bg-white p-6 shadow-[0_22px_60px_rgba(15,23,42,0.06)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-2 text-lg text-slate">{job.company}</div>
        </div>

        <span className="rounded-full border border-ink/10 bg-mist/45 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="rounded-[20px] bg-ink px-5 py-3 text-sm font-semibold text-white transition hover:bg-tide"
        >
          View Post
        </a>
        <button
          ...
          className="rounded-[20px] border border-ink/10 px-5 py-3 text-sm font-medium text-ink transition hover:border-ink/25 hover:bg-mist/45 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <div className="mt-6 rounded-[24px] border border-ink/8 bg-mist/35 px-5 py-5">
