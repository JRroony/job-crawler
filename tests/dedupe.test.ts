...
function createCandidate(overrides: Partial<Omit<JobListing, "_id" | "crawlRunIds">> = {}) {
  ...
  const sourceJobId = overrides.sourceJobId ?? "1";
  ...
  const canonicalUrl = overrides.canonicalUrl ?? "https://example.com/job";
  const resolvedUrl = overrides.resolvedUrl ?? "https://example.com/job/apply";
  const applyUrl = overrides.applyUrl ?? "https://example.com/job/apply";
